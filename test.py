...
fig.add_trace(go.Scatter(x=filtered_df['timestamp'], y=y, mode='lines+markers', name='Actual Energy Consumption', line=dict(color='blue')))

# Predicted Energy Consumption
fig.add_trace(go.Scatter(x=filtered_df['timestamp'], y=y_pred, mode='lines+markers', name='Predicted Energy Consumption', line=dict(color='red')))
...
fig.update_layout(
    ...
    xaxis=dict(
        tickformat="%H:%M",  # Show hours and minutes on x-axis
        tickmode="linear",
        tickangle=45,  # Rotate the x-axis labels for better readability
        dtick=900000000,  # 15-minute intervals in milliseconds (15 * 60 * 1000)
        range=[filtered_df['timestamp'].iloc[0], filtered_df['timestamp'].iloc[-1]]  # Set the range to show data from the first timestamp to the last
    ),
    autosize=True  # Ensure the graph takes up available space
)
...
# Fourth, scatter plot between Solar Irradiance and Energy Consumption with hue (Sunny or Not)
st.write("### Scatter Plot: Solar Irradiance vs Energy Consumption (Hue: Sunny or Not)")
fig3 = px.scatter(filtered_df, x='solar_irradiance_Wm2', y='energy_consumed_kWh', color='is_sunny', title="Solar Irradiance vs Energy Consumption",
                  labels={"solar_irradiance_Wm2": "Solar Irradiance (W/m²)", "energy_consumed_kWh": "Energy Consumed (kWh)", "is_sunny": "Sunny (0 = No, 1 = Yes)"})
...
fig4 = go.Figure()
fig4.add_trace(go.Scatter(x=filtered_df['timestamp'], y=filtered_df['energy_consumed_kWh'], mode='lines+markers', name='Energy Consumption', line=dict(color='blue')))
